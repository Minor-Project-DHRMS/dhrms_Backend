...
describe("Deployment", () => {
  before(async () => {
    ...
    ApproveDetails2_ContractFactory = await hre.ethers.getContractFactory("ApproveDetails2");
...
describe("Giving read permission", function () { 
  it("Should returns new doctor ID", async function () {
      expect(await DHRMS_PAT_Contarct.connect(patient1).giveReadAccess(doc1.address))
  ...
  it("Checking that doctor is able to get patient1 details", async () => {
      expect(await DHRMS_PAT_Contarct.connect(doc1).getPatientDetails(patient1.address)).to.equal(JSON.stringify(pd));
  });

  it("Checking that unauthorized doctors are not allowed to access the details", async () => {

      try {
          expect(await DHRMS_PAT_Contarct.connect(doc2).getPatientDetails(patient1.address)).to.equal(JSON.stringify(pd));
...
describe("Giving write permission", function () {
  it("Should returns new hospital ID to which permission given", async function () {
      ...
      expect(await DHRMS_PAT_Contarct.connect(patient1).giveWriteAccess(hospital1.address))
  ...
  it("Checking that unauthorized hospitals are not allowed to access the details", async () => {
      ...
      try {
          expect(await DHRMS_PAT_Contarct.connect(hospital2).getPatientDetails(patient1.address)).to.equal(JSON.stringify(pd));
          console.log("unauthorized access is happend");
          assert(false);
      } catch (error) {
          assert(true);
...
describe("Retrieving Doctor information", function () {
  it("Should return particular doctor details", async function () {
    ...
    expect(await DHRMS_DOC_Contarct.connect(doc1).getDoctorDetails(doc1.address)).to.eql(["Dr.M.N.Rayangoudar", "7611198762", "MBBS", "photo", "01/12/1968", "Cardiology"]);
  ...
  it("Should return patient list of particular doctor", async function () {
      ...
      console.log(await DHRMS_DOC_Contarct.connect(doc1).getPatientListOfDoc(doc1.address));
      expect(await DHRMS_DOC_Contarct.connect(doc1).getPatientListOfDoc(doc1.address)).to.eql([patient1.address]);

  });

  it("Should return hospital name in which doctor is working", async function () {

      expect(await DHRMS_DOC_Contarct.connect(doc1).getDoctorH(doc1.address)).to.eql(hospital1.address);
...
describe("Retrieving hospital information", function () {
  it("Should return particular hospital details", async function () {
      ...
      expect(await DHRMS_HOS_Contarct.connect(hospital1).getHospitalDetails(hospital1.address)).to.eql(["SDM Medical College Dharwad", "9203251212"]);

  });

  it("Should return doctor list of particular hospital", async function () {

      expect(await DHRMS_HOS_Contarct.connect(hospital1).getHospitalDoctorList(hospital1.address)).to.eql([]);

  });

  it("Should return patient list of particular hospital", async function () {

      expect(await DHRMS_HOS_Contarct.connect(hospital1).getHospitalPatientList(hospital1.address)).to.eql([patient1.address]);
...
describe("Retrieving patient information", function () {
  it("Should return particular patient details", async function () {
      ...
      expect(await DHRMS_PAT_Contarct.connect(patient1).getPatientDetails(patient1.address)).to.equal(JSON.stringify(pd));

  });

  it("Should return doctors list of particular patient", async function () {

      expect(await DHRMS_PAT_Contarct.connect(patient1).getDoctorsList(patient1.address)).to.eql([doc1.address]);

  });

  it("Should return hospital list of particular patient", async function () {

      expect(await DHRMS_PAT_Contarct.connect(patient1).getHospitalsList(patient1.address)).to.eql([hospital1.address]);
...
describe("Removing read permission", function () {
  it("Should returns new doctor ID to which permission reverted", async function () {
      ...
      expect(await DHRMS_PAT_Contarct.connect(patient1).removeReadAccess(doc1.address))
...
describe("Removing write permission", function () {
  it("Should returns new hospital ID to which permission reverted", async function () {
      ...
      expect(await DHRMS_PAT_Contarct.connect(patient1).removeWriteAccess(hospital1.address))
...
describe("Send Records for upload", function () {
  it("Should returns new uploaded records", async function () {
      ...
      expect(await DHRMS_DOC_Contarct.connect(doc1).sendRecordsForUpload("CID", patient1.address))
...
describe("Send Records for upload (Hospital)", function () {
  it("Should returns new uploaded recorded (Hospital)", async function () {
      ...
      expect(await DHRMS_HOS_Contarct.connect(hospital1).sendRecordsForUploadH("CID", patient1.address, hospital1.address))
...
describe("Adding records to patient history", function () {
  it("Should returns all records of patient", async function () {
      ...
      await DHRMS_HOS_Contarct.connect(hospital1).reportUploaded(patient1.address, "CID");

      expect(await DHRMS_PAT_Contarct.connect(hospital1).getRecordsHistory(patient1.address)).to.eql(["CID"]);
...
describe("Retrieving Government information", function () {
  it("Should return particular government office details", async function () {
      ...
      expect(await DHRMS_GOV_Contarct.connect(gov2).getGovernmentDetails(gov2.address)).to.eql(["Hubli-office", "9433387654"]);
